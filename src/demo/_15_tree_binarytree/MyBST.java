package demo._15_tree_binarytree;
...
public class MyBST {
    ...
    // insertIntoBST: add value into rootNode tree
    ...
    // Delete node key in tree node
    // Return: new tree has deleted node key
    public TreeNode deleteNode(TreeNode root, int key) {
        if (root == null) {
            return null;
        }
        // B1: Searching for delete node
        if (key < root.val) {
            root.left = deleteNode(root.left, key);
        } else if (key > root.val) {
            root.right = deleteNode(root.right, key);
        } else { // root.val = key => delete root | define delete node
            // B2: Delete root node
